...
def allaAtomerFinns(hashtabell, atomlista):
    ...
    for kontrollAtom in atomlista:
        ...
        vikt = float(vikt)
        try:
            hashadAtom = hashtabell.sök(namn) # modifierat
            if hashadAtom.data.vikt != vikt: # modififierat
...
def knasAtomFinnsInte(hashtabell):
    ...
    try:
        atom = hashtabell.sök(knasnamn) # modifierat
